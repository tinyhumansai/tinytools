...
pub struct ParsedToolCall {
    /// Canonical tool name supplied by the model.
    pub name: String,
...
/// Normalize an argument value, decoding stringified JSON when possible.
#[must_use]
...
/// Parse a tagged JSON value containing one or more tool calls.
#[must_use]
...
static TOOL_CALL_TAG_RE: LazyLock<Option<Regex>> =
    LazyLock::new(|| Regex::new(r"(?i)<[|/\s]*tool[_-]?call[|/\s]*>").ok());
...
fn normalize_garbled_tool_call_tags(s: &str) -> Cow<'_, str> {
    ...
    let Some(tool_call_tag_re) = TOOL_CALL_TAG_RE.as_ref() else {
        return Cow::Borrowed(s);
    };
    ...
    for &[open, close] in tags.as_chunks::<2>().0 {
        ...
        if let Some(recovered) = recover_sentinel_tool_call_body(stripped) {
...
fn parse_invoke_attribute_block(after_prefix: &str) -> Option<(ParsedToolCall, usize)> {
    static INVOKE_NAME_RE: LazyLock<Option<Regex>> =
        LazyLock::new(|| Regex::new(r#"name\s*=\s*"([^"]*)""#).ok());
    static PARAMETER_RE: LazyLock<Option<Regex>> = LazyLock::new(|| {
        Regex::new(r#"(?s)<parameter\s+name\s*=\s*"([^"]*)"\s*>(.*?)</parameter>"#).ok()
    });

    let invoke_name_re = INVOKE_NAME_RE.as_ref()?;
    let parameter_re = PARAMETER_RE.as_ref()?;
...
/// Parse GLM-style `tool/name>payload` calls from model text.
#[must_use]
...
pub fn parse_tool_calls(response: &str) -> (String, Vec<ParsedToolCall>) {
    ...
    loop {
        ...
        if let Some(start) = invoke_attr.filter(|_| use_invoke_attr) {
    ...
    if calls.is_empty() {
        static MD_TOOL_CALL_RE: LazyLock<Option<Regex>> = LazyLock::new(|| {
            Regex::new(
        ...
        for cap in MD_TOOL_CALL_RE
            .as_ref()
            .into_iter()
            .flat_map(|regex| regex.captures_iter(response))
        {
